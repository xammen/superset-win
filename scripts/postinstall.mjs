 ...
 *  3. Materialize the compiled Lingui catalogs (non-fatal; mirrors upstream
 *     postinstall.sh so a fresh clone can typecheck without turbo).
...
// Materialize the compiled Lingui catalogs. Generated, not committed — a
// direct `bun run --filter=<pkg> typecheck` bypasses turbo, so a fresh clone
// would hit "Cannot find module '../locales/en/messages'". Non-fatal: a
// missing translation must fail a real build, not an install.
tryRun("bun run --filter=@superset/i18n build", "i18n build");
