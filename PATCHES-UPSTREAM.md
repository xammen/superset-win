...
> **Status (2026-09-22):** base moved to upstream `desktop-v1.30.1`. Patches are
> committed directly to this repo (see the git history), this file is the record
> of what was changed and why. Two patches are now obsolete and were **dropped
> during the 1.30.1 rebase**:
> - **Patch 0** (pin Mastra past Bun's release-age window): upstream no longer
>   depends on `@mastra/core`/`mastracode` at all — the overrides were removed.
> - **Patch 19** (Windows quit confirmation at the window `close` event):
>   upstream now `preventDefault()`s the last window's close outside macOS and
>   routes it through `before-quit`, so the confirmation dialog runs while the
>   window still exists — the root cause patch 19 worked around. The
>   `!PLATFORM.IS_WINDOWS` guard it added to `before-quit` was reverted.
>
> Patch 34/35 (worker-thread parcel watcher + `windows` backend pin) were
> re-ported onto upstream's new `watch-backend/` abstraction: the offload now
> lives in `packages/workspace-fs/src/watch-backend/parcel-backend.ts`.
